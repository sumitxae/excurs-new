...
class DeviceDetailsActivity : AppCompatActivity() {
    ...
    private fun updateLoadingOverlay() {
        ...
        ) || !isAnalysisCompleted || isAnalyzingTemperatureFlag
        binding.loadingOverlay.visibility = if (shouldShow) View.VISIBLE else View.GONE
    }
    
    
    
    private fun plotTemperatureGraph(analysis: com.minew.sensormanager.data.models.TemperatureHistoryAnalysis) {
        val chart = binding.graphContainer
        ...
        val temperatureEntries = dataPoints.map { point ->
            Entry(point.timestamp.toFloat(), point.temperature.toFloat())
        }
        ...
        val timeDiff = if (temperatureEntries.size > 1) {
        ...
        if (temperatureEntries.size >= 2) {
            ...
            when {
                allNormal -> {
                    ...
                    val normalDataSet = LineDataSet(temperatureEntries, "Normal Temperature").apply {
                        color = normalBlue
                        lineWidth = 2f
                ...
                allExcursion -> {
                    ...
                    val excursionDataSet = LineDataSet(temperatureEntries, "Excursion Temperature").apply {
                        color = excursionRed
                        lineWidth = 2f
                        mode = LineDataSet.Mode.LINEAR
                        setDrawValues(false)
                    ...
                    if (firstExcursionIndex != -1 && firstExcursionIndex > 0) {
                        ...
                        val normalDataSet = LineDataSet(normalSegment, "Normal Temperature").apply {
                            color = normalBlue
                            lineWidth = 2f
                            mode = LineDataSet.Mode.LINEAR
                            setDrawValues(false)
                        ...
                        val excursionSegment = temperatureEntries.subList(firstExcursionIndex, temperatureEntries.size)
                        val excursionDataSet = LineDataSet(excursionSegment, "Excursion").apply {
                            color = excursionRed
                            lineWidth = 2f
                            mode = LineDataSet.Mode.LINEAR
                            setDrawValues(false)
                        ...
                        val fallbackDataSet = LineDataSet(temperatureEntries, "Temperature").apply {
                            color = primaryOrange
                            lineWidth = 2f
                            mode = LineDataSet.Mode.LINEAR
                            setDrawValues(false)
            ...
            val singleDataSet = LineDataSet(temperatureEntries, "Temperature").apply {
                color = primaryOrange
                lineWidth = 2f
                mode = LineDataSet.Mode.LINEAR
                setDrawValues(false)
        ...
        // Add circles only for: (1) first normal point, (2) first excursion after a normal, (3) last point
        ...
        val upperAlertDataSet = LineDataSet(upperAlertEntries, "High Threshold (8°C)").apply {
        ...
        val lowerAlertDataSet = LineDataSet(lowerAlertEntries, "Low Threshold (2°C)").apply {
    ...
    private fun observeViewModel() {
        viewModel.deviceInfo.observe(this) { deviceInfo ->
            deviceInfo?.let {
                ...
                binding.tvCurrentTemperature.text = it.temperature?.let { temp -> 
                    "${String.format("%.1f", temp)}°C" 
                } ?: "N/A"
                
                // Set excursion event time if available
                binding.tvExcursionEventAt.text = "Processing..." // Will be updated by excursionEventTime observer
    ...
    private fun showDiagnosticsDialog(diagnostics: String) {
        val dialog = androidx.appcompat.app.AlertDialog.Builder(this)
            ...
            .setPositiveButton("Copy to Clipboard") { _, _ ->
                ...
                val clip = android.content.ClipData.newPlainText("Connection Diagnostics", diagnostics)
